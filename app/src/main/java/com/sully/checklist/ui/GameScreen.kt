...
fun GameScreen(viewModel: GameViewModel = viewModel()) {
    ...
    BoxWithConstraints(
        ...
        if (isLandscape) {
            ...
            // "Display" cols for LAYOUT (actual columns needed: 4 for 12 cards, 5 for 15 cards)
            val displayCols = kotlin.math.ceil(cardCount / 3.0).toInt().coerceAtLeast(1)

            // 2. Calculate Available Space
            // Vertical: MaxHeight - VerticalPadding - GridVerticalPadding(16dp) - Spacing
            ...
            // Safe Padding (Notch, etc) must also be subtracted as the root container has
            // padding(safePadding)
            ...
            val finalCardWidth = kotlin.math.min(cardWidthByH.value, cardWidthByW.value).dp

            // Calculate final grid width based on DISPLAY cols (to layout 4x4x4 correctly for 12
            // cards)
            ...
            Row(
                    modifier = Modifier.fillMaxSize().padding(safePadding).padding(16.dp),
                ...
                Box(
                        modifier =
                                Modifier.weight(1f)
                                        .fillMaxHeight(), // Remove padding(end=16.dp) as it's
                    ...
                    LazyVerticalGrid(
                            ...
                            modifier =
                                    Modifier.widthIn(max = maxGridWidth).graphicsLayer {
                ...
                Column(
                        modifier =
                                Modifier.fillMaxHeight().width(120.dp), // Fixed width for controls
            ...
            // 3. Available Space
            // Vertical: MaxHeight - TopPad - BottomPad - BottomBar - Buffer(32dp) - GridPad(16dp)
            val availableHeight =
                    constraintsMaxHeight - topPadding - bottomPadding - bottomBarHeight - 48.dp
            ...
            val finalCardHeight = kotlin.math.min(cardHeightByH.value, cardHeightByW.value).dp
            ...
            Column(
                ...
                Column(
                    ...
                    Row(horizontalArrangement = Arrangement.spacedBy(16.dp)) {
                        ...
                        Button(onClick = { viewModel.onHintClicked() }) { Text("Hint") }
                        Button(onClick = { viewModel.startNewGame() }) { Text("New") }
                    }
                }
            }
        }
    ...
    Canvas(modifier = Modifier.fillMaxSize()) {
        ...
        val textPaint =
                android.graphics.Paint().apply {
                    color = android.graphics.Color.parseColor("#00332B") // Slightly darker than bg
                    textSize = 100f
        ...
        for (i in 0 until cols) {
            for (j in 0 until rows) {
                ...
                drawContext.canvas.nativeCanvas.drawText(patternText, x, y, textPaint)
